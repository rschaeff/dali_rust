...
2. **Rust reimplementation** (Phases 1-9) — compiled implementation validated
   ...
   expose the compiled pipeline to Python.
...
| PyO3     | 6/6 binding tests   |                      | 6/6 (100%)        |
...
| WOLF                | 3     | 5-struct + 18-struct + combined        |
| DP                  | 2     | 5-struct + 18-struct                  |
| DALICON             | 2     | 5-struct + 18-struct                  |
| PARSI               | 2     | 5-struct + 18-struct                  |
| FILTER95            | 2     | 5-struct + 18-struct                  |
| E2E pipeline        | 1     | 3 reference pairs, 10 DCCP entries    |
| Import pipeline     | 4     | PDB reader + full import (5+18 struct)|
| PyO3 bindings       | 6     | read_dat, ca_coords, import_pdb, store, compare_pair, wolf_path |
| **Total**           | **53**|                                       |
...
```
dali_rust/dali-core/
  src/                              8,964 lines (32 .rs files)
    types/        3 files,    223 lines — Protein, Segment, Alignment, DistMat
    numerics/     6 files,    975 lines — nint, Kabsch/u3b, fitz, NW, scoring
    io/           7 files,  1,556 lines — .dat parser, PDB reader, DSSP, secstr, domain, import
    wolf/         4 files,    824 lines — SSE spatial hashing
    dp/           1 file,     173 lines — Z-score computation
    dalicon/      1 file,   1,451 lines — GA refinement
    parsi/        6 files,  2,880 lines — Branch-and-bound
    filter95/     1 file,     490 lines — Redundancy filtering + FITZ
    pipeline.rs               239 lines — E2E orchestrator
    store.rs                  133 lines — ProteinStore (caching layer)
    lib.rs                     20 lines — Module declarations

  tests/                            1,961 lines (10 .rs files)
    test_import.rs            358 lines — PDB reader + import pipeline
    test_filter95.rs          257 lines — FILTER95 validation
    test_dp.rs                251 lines — DP validation
    test_wolf.rs              245 lines — WOLF validation
    test_dalicon.rs           206 lines — DALICON validation
    test_parsi.rs             201 lines — PARSI validation
...
dali_rust/dali-python/
  src/lib.rs                  213 lines — PyO3 bindings (4 wrapper types + 5 functions)
  test_bindings.py            105 lines — Python validation (6 tests)
```

Total: 11,243 lines Rust + Python bindings.
...
- **Ecosystem.** PyO3 for Python integration (Phase 9 delivers this), rayon for
  pair-level parallelism, pdbtbx for PDB/CIF parsing with gzip support. Fortran
  lacks this ecosystem.
...
6. **Translation is faster than discovery.** Phase 0 (Python teardown) took
   longer per module than Phases 1-8 (Rust reimplementation) because discovery
...
**Complete.** All 9 phases implemented, validated, and tested. The Rust
...
expose the full pipeline to Python with compiled performance.
...
| 9     | PyO3 Python Bindings           | 6/6 binding tests (100%)                |

47 automated Rust tests + 6 Python binding tests. All passing.

### Phase 9: PyO3 Python Bindings

Phase 9 wraps the compiled Rust pipeline in a Python module via PyO3, enabling
use from Python with compiled performance. The `dali` module provides three
tiers of API:
